...
public class AppointmentDao {
	...
	public int countApprovedAppointmentByDoctorId(int doctorId) {

		int count = 0;

		try {
			String sql = "select count(*) from appointment where doctor_id=? and status='Approved'";
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setInt(1, doctorId);
	...
	public List<Appointment> getTodayAppointmentsByDoctor(int doctorId) {
		...
		try {
			...
			String sql = "select * from appointment where doctor_id=? and appoint_date=CURDATE()";
